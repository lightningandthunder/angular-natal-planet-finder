...
fn get_natal_precession_input() -> anyhow::Result<Option<InputConfig>, Box<dyn std::error::Error>> {
    ...
    println!("Welcome to the Nova Natal Planet Finder tool!\nSource code may be found at https://github.com/lightningandthunder/rust_nova_api");
...
fn main() {
    ...
    let file_path = PathBuf::from("output")
        .join(format!("{}.txt", &config.title))
        .to_string_lossy()
        .to_string();
    let mut file = create_file(file_path.as_str()).unwrap();
