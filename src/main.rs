// Angular Natal Planet Finder, a Rust CLI application to find places
// where natal planets are foreground in some sidereal context.
// Copyright (C) 2024 Mike Verducci
/*This file is part of Angular Natal Planet Finder.

Angular Natal Planet Finder is free software: you can redistribute it and/or modify it under the terms of the GNU Affero General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.

Angular Natal Planet Finder is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero General Public License for more details.
...
use swiss_ephemeris_lib::{
    angular_precessed_planets_in_range, close_dll,
    file_utils::{create_file, write_to_file},
    open_dll,
    spacetime_utils::{
...
pub const PLANET_NAMES: [&'static str; 12] = [
...
fn get_natal_precession_input() -> anyhow::Result<Option<InputConfig>, Box<dyn std::error::Error>> {
    ...
    println!("Welcome to the Nova Natal Planet Finder tool!\nSource code may be found at https://github.com/lightningandthunder/angular-natal-planet-finder.git");
    ...
    let geocoded_coords = geocode(birth_location, API_KEY.to_string())?;
    ...
    let orb_raw: String = Input::with_theme(&theme)
        .with_prompt(
            "Please enter the orb (in degrees) to allow for angular planets, e.g. 1 or 2.5",
        )
        .default("1.0".to_string())
        .interact()?;

    let orb: f64 = orb_raw.parse()?;
...
fn press_any_key_prompt() {
    let theme = ColorfulTheme {
        values_style: Style::new().blue().dim(),
        ..ColorfulTheme::default()
    };
    Confirm::with_theme(&theme)
        .with_prompt("Press Y to exit")
        .interact()
        .unwrap();
}
...
fn main() {
    ...
    let config_result = get_natal_precession_input();
    if let Err(e) = config_result.as_ref() {
        println!("Error: {:?}", e);
        press_any_key_prompt();
    }

    let config_unwrapped = config_result.unwrap().unwrap();

    println!("Calculating...");

    let calculation_result = angular_precessed_planets_in_range(
        config_unwrapped.local_birth_dt,
        config_unwrapped.solunar_dt,
        range,
        config_unwrapped.allowed_body_numbers,
        config_unwrapped.target_planet_number,
        config_unwrapped.harmonic,
        config_unwrapped.orb,
    );

    if let Err(e) = calculation_result {
        println!("Error: {:?}", e);
        press_any_key_prompt();
        return;
    }
    ...
    let file_path = PathBuf::from("output")
        .join(format!("{}.txt", &config_unwrapped.title))
        .to_string_lossy()
        .to_string();
    let mut file = match create_file(file_path.as_str()) {
        Ok(f) => f,
        Err(e) => {
            press_any_key_prompt();
            return;
    ...
    let (selected_solunar_dt, angularities) = calculation_result.unwrap();
    let solunar_dt_string = format!("Solunar Return Date: {}\n\n", selected_solunar_dt);
    if let Err(e) = write_to_file(&mut file, solunar_dt_string) {
        println!("Error writing to file: {:?}", e);
        press_any_key_prompt();
    };
    angularities.into_iter().for_each(|x| {
        if let Err(e) = write_to_file(&mut file, format!("{}: {}\n", x.0, x.1)) {
            println!("Error writing to file: {:?}", e);
            press_any_key_prompt();
        }
    });
    ...
    println!(
        "Report written to {}.txt. Please check the current directory for your output file.",
        &config_unwrapped.title
    );
